...
class TestSyncProtocol(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.owner_id1 = "owner_1"
        cls.owner_id2 = "owner_2"
    ...
    def test_01_validate_diff(self):
        with self.assertRaises(Exception):
            self.mobync._Mobync__validate_diff({}, self.owner_id1)

        with self.assertRaises(Exception):
            self.mobync._Mobync__validate_diff(self.create_diff, self.owner_id2)

        self.assertTrue(self.mobync._Mobync__validate_diff(self.create_diff, self.owner_id1))
        self.assertTrue(self.mobync._Mobync__validate_diff(self.update_diff, self.owner_id1))
        self.assertTrue(self.mobync._Mobync__validate_diff(self.delete_diff, self.owner_id1))
    ...
    def test_05_apply_diff(self):
        ...
        res = self.mobync.synchronizer.read('diffs', [ReadFilter(Diff.OWNER, FilterType.equal, self.owner_id1)])
